...
pub fn bech32_with_relays(nip19: &Nip19, source_relays: &[RelayUrl]) -> Option<String> {
    ...
    match nip19 {
        Nip19::Event(ev) => {
            ...
            let mut new_event = Nip19Event::new(ev.event_id)
                .relays(source_relays.iter().cloned());
            ...
            new_event.to_bech32().ok().or_else(|| nip19.to_bech32().ok())
        }
        Nip19::Coordinate(coord) => {
            let new_coord =
                Nip19Coordinate::new(coord.coordinate.clone(), source_relays.iter().cloned());
            new_coord.to_bech32().ok().or_else(|| nip19.to_bech32().ok())
        }
